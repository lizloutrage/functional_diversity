...
# Define randomization function for assemblages
randomize_assemblages <- function(asb_matrix) {
  return(asb_matrix[sample(rownames(asb_matrix)), ])
}

# Names of functional diversity indices
indices_names <- c("fdis", "feve", "fric", "fdiv")
...
for (sim in 1:n_simulations) {
  ...
  # Randomize assemblages
  randomized_assemblages <- randomize_assemblages(depth_fish_biomass)
  
  # Compute functional distance and space
  ...
  simFD_cal <- mFD::alpha.fd.multidim(
    ...
    asb_sp_w = randomized_assemblages,
...
for (index_name in indices_names) {
  ...
  SES_FD[, index_name] <- ifelse(
    sdNullFD[, index_name] == 0,
    NA,  # Handle zero variance by assigning NA
